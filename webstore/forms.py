...
class Shipping(forms.Form):
    ...
    last_name = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Enter your last name'}), help_text="Required. Must be 30 characters or less", max_length=30)
    email = forms.EmailField(widget=forms.EmailInput(attrs={'placeholder': 'Enter a valid email address'}), help_text="Required.")
    phone_number = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Enter your phone number'}), help_text="Required.", max_length=15)
    street = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Enter your street address'}), help_text="Required.", max_length=50)
    state = forms.ChoiceField(choices=[('', 'Select a state')] + list(Info.STATE_CHOICES), widget=forms.Select(attrs={'class': 'form-control'})
    )
    zipcode = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Enter your zipcode'}), help_text="Required.", max_length=9)
    ...
    expiration = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Enter card expiration date DD/YY'}), help_text="Format DD/YY", max_length=5)
